/*************************************************************
  ...
  Develop a basic W3.CSS interface.
    ...
    v15 tailored by Wina 2022
    v16 tailored by WIna 2023 to also fit the gtn game
*************************************************************/
...
function ad_user() {
  ...
  document.getElementById("b_adLobby").style.backgroundColor = "grey";
  document.getElementById("b_adActive").style.backgroundColor = "grey"; 
  // ENSURE THE READ FUNCTION NAME & THE PATH NAME ARE CORRECT
  fb_readAllSnapshot(DETAILS, ad_processUSERReadAll);
}
...
function ad_score() {
  ...
  document.getElementById("b_adActive").style.backgroundColor = "grey"; 
  // ENSURE THE READ FUNCTION NAME & THE PATH NAME ARE CORRECT
  fb_readAllSnapshot(SCORES, ad_processBBReadAll);
}
...
/**************************************************************/
function ad_lobby() {
  console.log('ad_lobby: ');
  
  document.getElementById("b_adUser").style.backgroundColor = "grey";
  document.getElementById("b_adHome").style.backgroundColor = "grey";
  document.getElementById("b_adScore").style.backgroundColor   = "grey";
  document.getElementById("b_adLobby").style.backgroundColor = "cyan";
  document.getElementById("b_adActive").style.backgroundColor = "grey"; 
  // ENSURE THE READ FUNCTION NAME & THE PATH NAME ARE CORRECT
  fb_readAllSnapshot(LOBBY, ad_processLobbyReadAll);
}
...
function ad_active() {
  ...
  document.getElementById("b_adScore").style.backgroundColor   = "grey";
  document.getElementById("b_adLobby").style.backgroundColor = "grey";
  document.getElementById("b_adActive").style.backgroundColor = "cyan"; 
  // ENSURE THE READ FUNCTION NAME & THE PATH NAME ARE CORRECT
  fb_readAllSnapshot(ACTIVE, ad_processActiveReadAll);
}
...
function ad_game(){
  ...
  fb_readAllSnapshot(LOBBY, ad_processGAMEReadAll);
  ...
  if (_result == 'OK') {
    _dbRec.forEach(function(childSnapshot) {
      ...
      ad_adminArray.push({     
        ...
        bbScore       : childData.bbScore,
        tttPlayer1Win : childData.tttPlayer1Win,
        tttPlayer2Win : childData.tttPlayer2Win,
        tttTie        : childData.tttTie,
        gtnLose       : childData.gtnLose,
        gtnWin        : childData.gtnWin
      });
    });
...
/**************************************************************/
// ad_processLobbyReadAll(_result, _dbRec)
// Called by db_readAll to handle result of read ALL BB records request.
// Save data & update display with record info
// Input:  read status "OK", data record just read. 
//         NOTE: this is the raw data, EG snapshot, NOT snapshot.val()
// Return:
/**************************************************************/
function ad_processLobbyReadAll(_result, _dbRec) {
  console.log('ad_processLobbyReadAll: ', 'result = ' + _result);
...
/**************************************************************/
// ad_processActiveReadAll(_result, _dbRec)
// Called by db_readAll to handle result of read ALL BB records request.
// Save data & update display with record info
// Input:  read status "OK", data record just read. 
//         NOTE: this is the raw data, EG snapshot, NOT snapshot.val()
// Return:
/**************************************************************/
function ad_processActiveReadAll(_result, _dbRec) {
  console.log('ad_processActiveReadAll: ', 'result = ' + _result);

  if(_result =='FAIL'){
  
  var tableInfo = document.getElementById("t_userData"); //Get info on target table

  tableInfo.textContent = "No Record";
  }
  ...
  if (_result == 'OK') {
    _dbRec.forEach(function(childSnapshot) {
      ...
      ad_adminArray.push({     
        uid           :  childKey,
        p1gameName    :  childData.p1gameName,
        p1uid         :  childData.p1uid,
        p1guess       :  childData.p1guess,
        p1win         :  childData.p1win,
        p1lose        :  childData.p1lose,
        range         :  childData.range,
        p2gameName    :  childData.p2gameName,
        p2uid         :  childData.p2uid,
        p2guess       :  childData.p2guess,
        p2win         :  childData.p2win,
        p2lose        :  childData.p2lose,
      });
    });
...
/**************************************************************/
// ad_processGAMEReadAll(_result, _dbRec)
